...
현재 버퍼에서 hunk 단위 변경을 빠르게 확인/이동하는 데 집중한다. gutter 표시(`│`, `_`, `‾`, `~`)로 맥락을 즉시 보여주고, preview/blame/diff를 같은 편집 흐름 안에서 처리한다. 즉, 파일 내부 미세 변경 추적은 gitsigns가 전담한다.

### `tpope/vim-fugitive`
`:Git` 인터페이스를 중심으로 status, blame, log, 파일 diff 같은 “Git 조작”을 텍스트 기반으로 통합한다. gitsigns가 hunk 관찰 도구라면, fugitive는 명령 중심의 작업 허브에 가깝다. 브랜치/인덱스 상태를 확인하고 조작하는 일상 작업에 최적화했다.

### `sindrets/diffview.nvim`
브랜치 간 비교, 커밋 히스토리 탐색, 프로젝트 범위 diff를 시각적으로 다룬다. `diff2_horizontal`, `diff3_mixed`, 트리형 파일 패널(width=35)로 비교 맥락을 명확히 보여준다. fugitive가 명령형 진입점이라면 diffview는 결과를 넓게 검토하는 뷰어 역할이다.

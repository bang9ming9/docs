...
- [Treesitter textobjects 이동 메모](#treesitter-textobjects-이동-메모)
- [단축키 빠른 정리](#단축키-빠른-정리)
...
3. **브랜치 단위/큰 범위 비교 — `diffview.nvim`**
   ...
   - 현재 설정 기준 기본 비교는 `main...HEAD`이며, `:DiffviewOpen main...HEAD`처럼 기준점을 명시해 연다.
...
자동완성이 뜰 때는 “후보 이동 → 확정/취소 → 문서 스크롤” 순서로 최소 조작만 반복한다.

- 후보 다음/이전: `<Tab>`, `<S-Tab>`
- 확정: `<CR>`
- 수동으로 completion 열기: `<C-Space>`
- completion 닫기: `<C-e>`
- 문서 스크롤: `<C-f>`, `<C-b>`

마크다운 렌더링 안정성을 위해 키 표기는 모두 인라인 코드로 고정했다.
...
## Treesitter textobjects 이동 메모

현재 문서에서는 textobjects 이동을 “함수/인자 경계를 빠르게 오간다”는 흐름으로만 기록한다.

- 실제 이동 키는 로컬 설정의 `nvim-treesitter-textobjects` 매핑에 따른다.
- 이 문서에는 키를 단정하지 않고, 설정에 정의된 키를 기준으로 함수/파라미터 경계를 이동한다는 사용 맥락만 유지한다.
...
| `<leader>dm` | 기본 브랜치(`main`)와 비교 (`:DiffviewOpen main...HEAD`) | **d**iff vs default branch (**m**ain) | Normal |

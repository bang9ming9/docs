...
이 문서는 현재 Neovim 설정에서 사용하는 작업 흐름을 정리한 메모다. 코드를 읽고 수정하고 문서로 옮기는 과정에서 사용하는 조작을 중심으로 정리한다.

현재 문서에 정리된 흐름은 다음과 같다.
...
`<leader>`는 현재 Neovim 기본값인 `\`로 사용한다.

## Vim 키 조합

Vim 키는 보통 `동작 + 대상` 구조로 조합된다.
...
예를 들어 함수 호출부에서 `gd`로 구현을 확인한 뒤, `Ctrl+o`로 다시 호출부로 돌아온다.
...
프로젝트 전체 검색은 Telescope를 사용하고, 현재 파일 안에서 같은 단어를 찾을 때는 Vim 기본 검색을 사용한다.
...
예를 들어 변수명 위에 커서를 두고 `*`를 누르면 현재 파일에서 같은 단어를 검색한다.

## Git diff 확인하기

변경사항 확인은 범위에 따라 도구를 나눠서 사용한다. 작업 중인 파일의 변경은 gitsigns로 보고, 브랜치나 PR 단위 변경은 diffview로 본다.
...
| `\dm` | master 기준 비교 | diff master |
| `\dB` | 두 브랜치 비교 | diff branches |
| `]c` | 다음 변경 블록 | next change |
| `[c` | 이전 변경 블록 | previous change |

Diffview 안에서 `]c`, `[c`는 diff의 변경 블록 단위 이동에 사용한다. `{`, `}`는 Vim의 문단 이동이다.

GitHub PR처럼 base 브랜치와 target 브랜치를 비교하려면 `\dB`를 사용한다. 이때 내부적으로는 `base...target` 형태의 triple-dot 비교를 사용한다. `\dm`은 현재 설정에서 `master` 기준 비교로 고정되어 있다.

## 코드 검색하기

코드 검색은 Telescope를 사용한다. 현재 설정에는 파일, 문자열, buffer, 최근 파일, 심볼, 진단 검색 키가 등록되어 있다.
...
이 키는 같은 값을 여러 곳에 붙여넣을 때 기본 레지스터를 유지한다.

## Visual 선택 복원과 문단 단위 조작

문서 작성이나 코드 블록 정리 중에는 방금 선택한 영역을 다시 선택하거나, 문단 단위로 조작한다.
...
`gv`는 선택 영역을 조작한 뒤 다시 같은 영역을 선택한다. 예를 들어 visual mode에서 한 번 들여쓰기한 뒤 같은 영역을 다시 들여쓰기하거나, 복사 후 같은 범위를 다시 편집할 때 사용한다.

## 같은 단어를 여러 개 선택하기

VSCode의 `Cmd+D`처럼 같은 단어를 하나씩 추가 선택하는 동작은 현재 설정에서 `vim-visual-multi`가 담당한다.
...
프로젝트 심볼 이름 변경에는 LSP rename인 `\rn`을 사용한다. Go에서는 `gopls`가 심볼 정보를 제공한다.

같은 단어 중 일부를 동시에 수정할 때는 `vim-visual-multi`를 사용한다. 현재 설정에서는 `<C-n>`이 visual-multi 단어 선택 키로 남아 있고, Neo-tree 토글은 `<leader>e`에 매핑되어 있다.
...
Markdown preview는 localhost에 바인딩되며 자동 시작되지 않는다. 현재 설정에서는 `\mp` 또는 Markdown preview 명령으로 수동 실행한다.

## 문제가 생겼을 때 확인할 명령

동작하지 않는 키가 있을 때는 아래 명령으로 관련 상태를 확인한다.
...
| `:TSUpdate` | Treesitter parser 업데이트 |

예를 들어 `gd`가 동작하지 않으면 `:LspInfo`로 LSP가 붙었는지 확인한다. `"+yaf`가 동작하지 않으면 Treesitter textobjects 설정 상태를 확인한다. `<C-n>`이 기대와 다르게 동작한다면 Neo-tree와 visual-multi 키 설정을 확인한다.

## 정리

이 문서는 현재 Neovim 설정에서 사용하는 작업 흐름을 기준으로 정리한 메모다.
...
이 문서는 `~/.config/nvim/init.lua`의 키맵과 플러그인 설정을 기준으로 작성되어 있다.

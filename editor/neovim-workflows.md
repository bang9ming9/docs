# Neovim 사용 흐름 메모
이 문서는 [`neovim-setup.md`](./neovim-setup.md) 구성을 바탕으로, 실제 작업에서 반복해서 쓰는 조작 흐름을 정리한 메모다.

구성과 설정값은 setup 문서를 참고하고, 여기서는 “언제 어떤 흐름을 고르는지”를 중심으로 정리한다.
...
- `<C-f>` / `<C-b>`: 후보 문서 스크롤
- 현재 설정은 `<Tab>`/`<S-Tab>`을 후보 이동 키로 쓰지 않고, 탭 문자는 기본 입력으로 둔다.

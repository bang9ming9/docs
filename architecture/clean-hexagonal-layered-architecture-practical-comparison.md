...
## 2) 한눈에 보는 차이
아래 요약은 "코드가 실제로 어디를 의존하는가"를 빠르게 구분하기 위한 메모다.
...
[Hexagonal]
inbound adapter (HTTP, gRPC, consumer)
  -> usecase/application
  -> outbound port
  -> outbound adapter (DB, external API, broker)
...
| 관점 | 레이어드 | 헥사고날 | 클린 |
|---|---|---|---|
| 초점 | 계층 분리와 호출 흐름 | 포트/어댑터 경계 | 의존성 규칙(바깥 -> 안쪽) |
| 처음 읽을 때 체감 | 가장 직관적 | 경계 개념 이해 필요 | 계층+의존성 규칙 동시 이해 필요 |
| 외부 기술 교체 대응 | 구조에 따라 편차 큼 | 포트 설계가 맞으면 유리 | 경계 설계가 맞으면 유리 |

### 2-3. Mermaid 미니 다이어그램
```mermaid
flowchart LR
  subgraph Layered
    L1[Handler] --> L2[Service] --> L3[Repository] --> L4[DB]
  end
```
...
flowchart LR
  subgraph Clean
    C1[Framework/Driver] --> C2[Interface Adapter] --> C3[UseCase] --> C4[Entity]
  end
```
...
### 단점(조건 포함)
- 도메인 규칙과 외부 연동이 계속 `service`에 모이면 `service`가 비대해지기 쉽다.
- 계층 분리만 있고 의존성 방향이나 도메인 경계를 관리하지 않으면 DB 모델 중심으로 로직이 흘러가기 쉽다.
...
- 단순히 모든 repository에 인터페이스를 추가하는 것만으로 설계가 좋아지지는 않는다.
- 추상화가 실제 변화 지점을 반영하지 못하면 파일 수만 늘고 유지보수성이 오히려 떨어질 수 있다.
...
| 인터페이스/포트가 필요한 이유 | 선택적(변경 지점 중심 권장) | 외부 의존성 교체/격리 경계 명시 | 안쪽 규칙 보호 + 구현 세부 분리 |
...
- 클린: 계층과 의존성 규칙을 더 명시적으로 설명

그래서 실제 프로젝트에서는 둘의 용어와 폴더 구조가 섞여 쓰이는 경우가 많다. 예를 들어 "클린 아키텍처 기반인데 repository port/adapter를 둔다" 같은 식이다.
...
## 9) 짧은 결론
레이어드는 단순성과 직관성을 얻기 쉽지만, 경계 관리가 약해지면 핵심 로직이 외부 기술이나 DB 모델 중심으로 끌릴 수 있다.

클린/헥사고날은 변경과 테스트를 위해 초기 복잡성을 감수하는 구조다. 다만 그 복잡성은 실제 변화 지점과 도메인 경계를 반영할 때 의미가 생긴다.

결국 중요한 것은 아키텍처 이름이 아니라, 핵심 비즈니스 로직이 외부 기술(프레임워크, DB, 메시지 브로커)에 끌려가지 않도록 경계를 관리하는 일이다. 모든 기능에 같은 수준의 추상화를 적용하기보다, 변경 가능성이 높거나 도메인 규칙이 중요한 부분부터 점진적으로 적용하는 편이 실무적으로 안전하다.

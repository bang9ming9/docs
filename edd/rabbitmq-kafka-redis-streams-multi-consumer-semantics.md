...
- 한 queue의 consumer가 멈추면 그 queue에만 메시지가 쌓이고, 다른 queue 소비에는 직접 영향이 없다.
...
- 따라서 새 consumer group을 추가하면, 기존 group의 처리 흐름 자체에는 보통 직접 영향 없이 같은 메시지를 별도로 읽을 수 있다.
...
- group은 마지막 전달 지점(last-delivered-id)과
...
결론적으로, “같은 메시지를 여러 소비자가 받아야 한다”는 문장을 만나면 먼저 시스템 이름보다 **독립 소비 단위를 어디에 둘지**부터 결정하는 편이 안전하다. 이 판단이 아키텍처 유연성과 장애 격리 수준을 거의 결정한다.

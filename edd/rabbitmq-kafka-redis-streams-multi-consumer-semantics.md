...
- 한 queue의 consumer가 멈추면 적체는 해당 queue에 국소화되고, 다른 queue의 ack 진행은 별도로 관찰된다.
...
- group은 전달 진행 기준(last-delivered-id 등)과
...
결론적으로, “같은 메시지를 여러 소비자가 받아야 한다”는 문장을 만나면 먼저 시스템 이름보다 **독립 소비 단위를 어디에 둘지**부터 결정하는 편이 안전하다. 이 판단이 아키텍처 유연성과 장애 격리 수준을 크게 좌우한다.
